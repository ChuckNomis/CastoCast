...
function ActorSearch() {
  ...
  const handleSearch = async (event) => {
    ...
    try {
      ...
      const response = await axios.get(`https://api.themoviedb.org/3/search/person?api_key=${apiKey}&query=${searchQuery}`);
    ...
    <div className="App">
      ...
      {selectedActor && (
        <div>
          ...
          <img src={`https://image.tmdb.org/t/p/w500${selectedActor.profile_path}`} alt={selectedActor.name} />
        </div>
      )}
      <h2>Movies</h2>
      <ul>
